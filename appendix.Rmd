...
dfcan <- st_calc(df3 %>% group_by(year, canton, type))
...
dfcan1 <- prop_calc(x = df3 %>% group_by(year, canton, type), 
                     y = dfcan, q = c("year", "canton", "type"))
```
The following code book explains the different variables in detail:
```{r, echo = FALSE}
...
```{r}
p <- ggplot(dat = dfprov1 %>% filter(province == "total"), aes(x = year, y = prop.students, col = type))
...
p <- ggplot(dat = dfprov1 %>% filter(province == "total"), aes(x = year, y = sum.teachers, col = type))
p1 <- p + geom_point() + geom_line()
p1
```
...
```{r, warning = FALSE}
p <- ggplot(dat = dfcan2019, aes(y = stratio, x = log(prop1))) + theme_bw() 
