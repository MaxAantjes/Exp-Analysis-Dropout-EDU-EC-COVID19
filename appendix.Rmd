...
prop_calc <- function(x, y, q){
        z <- x %>%
            ...
            select(-total) %>%
            left_join(y, by = q) %>%
            mutate(stratio = missing_to_zero(stratio)) %>%
            ungroup()
        return(z)}
...
#dfprov2 <- xts(x = dfprov1[, 2:length(dfprov1)], order.by = dfprov1$year) 
#dfcan2 <- xts(x = dfcan1[, 2:length(dfcan1)], order.by = dfcan1$year) 
```
The following code book explains the different variables in detail:
```{r, echo = FALSE}
...
```{r}
p <- ggplot(x = dfprov1, aes(x = year, y = ))
```
